const VoiceRecorder = function (options) {
    // Default options
    this.options = {
        fftSize: 512,
        bufferLen: 512,
    ...
    this.scriptProcessorNode.onaudioprocess = function(event) {
        ...
        if (self.isRecording) {
            self.recording.channel.push(new Float32Array(event.inputBuffer.getChannelData(0)));
            self.recording.recordingLength += self.options.bufferLen;
        }
        else {
            self.recording.tempChannel.push(new Float32Array(event.inputBuffer.getChannelData(0)));
            self.recording.recordingLength += self.options.bufferLen;

            if (self.recording.tempChannel.length > 50) {
                self.recording.tempChannel.shift();
                self.recording.recordingLength -= self.options.bufferLen;
            }
        }
    ...
    this.monitor = function() {
        ...
        if(start && !this.vadState) {
            ...
            if (this.recording.tempChannel.length) {
                this.recording.tempChannel.forEach(v => {
                    this.recording.channel.push(v);
                });

                this.recording.recordingLength += this.recording.tempRecordingLength;
                this.recording.tempChannel = [];
            }
        ...
        if(end && this.vadState) {
            ...
            this.options.voice_stop(createWAV(this.recording.channel, this.recording.recordingLength, this.options.sampleRate));
            this.recording.channel = [];
            this.recording.recordingLength = 0;
        }
    ...
    function createWAV(channel, length, sampleRate) {
        const interleaved = flattenArray(channel, length);
